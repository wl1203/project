...
extern float LoadVolt2,LoadVolt1;
extern STEP 	Step[16];
...
{	uint16 nn,ii;
  ...
  if(a==0)                    //出厂工艺
	...
	/************************************************加入部分*/
    voltdir1=(s_buff[nn]==0x30)?1
		      :(s_buff[nn]==0x31)?2:3;
		 nn+=1;
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],5);nn+=5;   
		LoadVolt1 = atof(TRBuff);
	...
	in_rot.onoff=s_buff[nn++]&0x0f;     //内循环设置
	in_rot.onoff=0;
	in_rot.start=s_buff[nn++]&0x0f;
	in_rot.start=0;
	in_rot.end=s_buff[nn++]&0x0f;
	in_rot.end=0;
	memset(TRBuff,0,sizeof(TRBuff)); 
	memcpy(TRBuff,&s_buff[nn],3); nn+=3; 
	in_rot.times=atoi(TRBuff);          //内循环次数
	in_rot.times=0;
	in_rot.idlemode=s_buff[nn++]&0x0f;
	in_rot.idlemode=0;
	memset(TRBuff,0,sizeof(TRBuff)); 
	memcpy(TRBuff,&s_buff[nn],4); nn+=4; 
	in_rot.endvalue=atoi(TRBuff);
	in_rot.endvalue=0;
		
	EndMode=s_buff[nn++]&0x0f;
	EndMode=1;
	memset(TRBuff,0,sizeof(TRBuff)); 
	memcpy(TRBuff,&s_buff[nn],5); nn+=5; 
	EndTime=atof(TRBuff);
	...
	Step[0].endvalue=Step[0].endvalue<10?10:Step[0].endvalue;//不足10s强行设定10s
	...
	{	
	     Step[5].endvalue=Step[5].endvalue<10?10:Step[5].endvalue;     //不足10s强行设定10s
	      if(Step[0].endvalue)
			Step[4].endvalue=0.5;//插入了第5个阶段	
	}
	else
	...
	{	nn=0;
		...
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],4); nn+=4;    //    8		 4-7		
		PidPara2.pid.igain=atoi(TRBuff);
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],4); nn+=4;    //		12   8-11
		PidPara2.pid.dgain=atoi(TRBuff);        //atoi()函数是将字符串转换成整型数的函数
		LoadType2=(s_buff[nn]==0x35)?5          //加载方式   12
			:(s_buff[nn]==0x37)?7:0;
		nn=nn+1;
		
		voltdir=(s_buff[nn]==0x30)?1
		      :(s_buff[nn]==0x31)?2:3;
		 nn+=1;
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],4);nn+=5;   
		//if(voltdir==1)
		  LoadVolt2 = atof(TRBuff)*100;
		//else if(voltdir==2)
			//LoadVolt2=-atof(TRBuff);
		                                 //这里需要添加电压    备用

		//nn=19+ii*11;			 
		for(ii=0;ii<20;ii++)
		{		
			Step2[ii].RunMode=s_buff[nn++]&0x0f;
			memset(TRBuff,0,sizeof(TRBuff)); 
			memcpy(TRBuff,&s_buff[nn],5); nn+=5; 		
			Step2[ii].LoadValue=atof(TRBuff);
			Step2[ii].LoadMode=s_buff[nn++]&0x0f;
			memset(TRBuff,0,sizeof(TRBuff)); 
			memcpy(TRBuff,&s_buff[nn],4); nn+=4; 
			Step2[ii].EndValue=atof(TRBuff);					
		}//nn=239
