...
STEP 	Step[16];
STEP2  Step2[22];
uint16 LoadType2=0,LastStep2=0,voltdir=0,voltdir1=0,LastStep1=0;
uint16  LoadVolt2,StartTime;
float TestVolt1,StartVolt1,LoadVolt1,TestVolt3,StartVolt3,volt,StartVolt2,TestVolt2;
IN_ROT 	in_rot;
...
volatile uint16 FlagMs2=0,FlagMs100=0,mS100=0,mS005=0;
...
void ReLoad2(uint16 a)
{
	...
	if(a==0)
	{	for(n=0;n<15;n++)
		{	IRcvStr(0xa4, n*16,(s_buff+n*16),16);		}  //16字节页写入功能
	}
	else
	{	for(n=0;n<15;n++)
	...
	while (1)
	{
		...
		switch(KeyValue)
		{
			...
			{	
				...
				V32.f32=Cons[7];
				ISendStr(0xa0,0x9c,V32.b8,4);Delay(2);     //地址需要改动
				V32.f32=Cons[8];
...
{		uint32 TotalTime;
		...
		{	
			...
			HTAB=0x10;putString("       ");	
			HTAB=0x17;putString("       ");
			while(Counter<30)                //这里是3S时间        就只是显示一下
			{	Read_Key();
				if(FlagMs2)
				{	Read_ResultM();		}	
		...
		volt=Cons[9];
		if(StartMotor(Cons[7],0))       //接着就是自启动  返回值为1代表F3返回
		{
			nowClock=0;STAB=0;In_pause=0;
			if(a==0)             //a的引入是 ConstantTest2(uint16 a)的a
				showRun0();
			else
			{   showPG00();
				DispPG00();
		...
		while(1)
		{
			...
			if((KeyValue==F1)&&(!End_Test))  //面板上F1是暂停键     !End_Test表明是在测试过程中
			{
				...
				else if(In_pause==0)		//按下F1一次后In_Test设置为0了，所以再按F1会执行else语句
				{	
					VTAB=0x70;HTAB=0x02;
					verString( "暂停");
				...
				{	VTAB=0;HTAB=0x15;
					...
					{	VTAB=0x70;
						HTAB=0x00;putString("F1");verString( "继续");putString("    ");
						HTAB=0x08;putString("       ");	
						HTAB=0x10;putString("F3");verString( "返回");
					}
				}
						...
						{	U0THR = MyID|0x01;
							...
							Bell();
							 ...
							 if(In_pause==0)      //没有暂停
					            ...
					            volt=Cons[9];             //启动电压
					           if(StartMotor(Cons[7],0)==0)
					           {	In_pause=0;           //这很关键，决定再次启动需要重发0x91命令
						            volt=Cons[8];         //试验电压
						            RunMotor(1, RunMode);
						            sprintf(sbuff,"%3d:%02d:%02d",shi,fen,miao);			
						            VTAB=0;HTAB=0x15;
   ...
   if(STAB==9)                   //启动电压
   	{
   	     switch(BTAB)
			{	case 0: case 1: //这里做出判断是为了跳过小数点
				{	HTAB=0x0a+BTAB;verEng(sbuff[BTAB]);   //显示而已
				 	break;	}	
				 default:
				 {	HTAB=0x0b+BTAB;verEng(sbuff[BTAB+1]);
				 	break;	}
...
{	float XFx;
	...
	{	
		VTAB=0x48;	
		sprintf(sbuff,"%2d",Cycle);     //状态是从0开始的
...
/*
	...
	/*else if(Step[0].loadtype=='3')                //代表转矩递增
	{	pid_tune((float)PidPara.pid.pgain,
				(float)PidPara.pid.igain,
				(float)PidPara.pid.dgain,4,0);
	}*/
	else                                     //负载率
	{	pid_tune(0,0,0,5,0);	
		pid0.runtype=5;
	...
	{   
		...
		{                        
			Bell();
			if(In_Test)                    //这一步执行必须满足In_Test=1条件 说明是在测试中
			{	
			    In_Test=0;
		...
		{	FlagPid=0;
			if(In_Test)
			{	if((pid0.runtype==0)&&(pid0.SetPoint2==0))//执行PID计算的条件
							pid_calc();                   //PID计算
			}
		}
	/*	
		...
		{	FlagMs100=0;	
			...
			{	++Counter;                 //计数     每100MS增加1		
				if(Counter>=Finish1)       //条件成立说明一个阶段测试结束
				{
					SetS+=dS;
					GoNext(); 
				}              //改变的是Cycle	和调用RunStep()函数执行下一个阶段
				else
				{	VTAB=0x5a;
					sprintf(sbuff,"%5.1f",(float)Counter/10);
					HTAB=0x19;putString(sbuff);	
...
{		float result;		
		...
		{	
            ...
            {	
		         ...
		         volt = StartVolt1;
				 RunMode = 1;
		         if(StartMotor(StartTime,2)==0)    //接着就是自启动  返回值为1代表F3返回
		         	{
		         	In_pause=0;In_Test=1;
					VTAB=0x00;HTAB=0x19;putString("     ");
					volt = TestVolt1;
		            RunMotor(1, RunMode);
					VTAB=0x70;
					HTAB=0x00;putString("F1");verString( "停止");
			...
			{	
				...
				break;			}	
			case 4: 
			{	RunMotor(0,1);RunMode=0;   //工艺会设定第5阶段的时间 
		...
		if(Step[Cycle].loadtype=='2')         //定转矩
		{
			LoadMode2=0;                      //标志
			pid0.SetPoint= Step[Cycle].loadvalue;    //直接把转矩值赋给
		...
		else if(Step[Cycle].loadtype=='5')              //定负载率
		{	LoadMode2=0;
		    pid0.SetPoint= Step[Cycle].loadvalue;
		    if(pid0.integral<0) pid0.integral=0;
		    pid0.SetPoint2=1;	
		...
		{
		    LoadMode2=1;                        //标志有效
			if(Step[Cycle].loadtype=='3')       //转矩递增
			{	SetS= Step[Cycle].loadvalue;    //设定的加载值
				dS=(SetS-pFt)/((float)Finish1);  //等分
				SetS=pFt+dS;
				pid0.SetPoint=SetS;
				pid0.SetPoint2=0;
			}
			else if(Step[Cycle].loadtype=='6')   //负载率
			{	SetS= Step[Cycle].loadvalue*10;
				dS=(SetS-LastLoad2)/((float)Finish1);
				SetS=LastLoad2+dS;
				result=SetS;
				result=(result > 1000.0) ? 1000.0 : (result < 0.0 ? 0.0 : result);
				LastLoad2=result;
...
{	
In001:
	//++Cycle;                         //最后Cycle 是会变成10的
	if(++Cycle<LastStep1+1)
...
{
	    IRcvStr(0xa0, 0xa8,V32.b8,4);
		StartTime=V32.f32;
		IRcvStr(0xa0, 0xac,V32.b8,4);
		TestVolt3=V32.f32;
		IRcvStr(0xa0, 0xb0,V32.b8,4);
		StartVolt3=V32.f32;
        ManualSetingDisplay();
		showManualSeting();
		...
		case GO_L://左移动
			...
			default:
			{
				 if(KeyValue<=9)
				{
					...
					{	
						case 0:
						{	sprintf(sbuff,"%03.0f",(float)StartTime);
							sbuff[BTAB]=KeyValue+0x30;
							StartTime=atof(sbuff);
							BTAB=(BTAB==2)?0:BTAB+1;
							break;
						}
						 case 1:
						 {
						 	sprintf(sbuff,"%04.1f",TestVolt3);
								if(BTAB<2)
									sbuff[BTAB]=KeyValue+0x30;
								else
									sbuff[BTAB+1]=KeyValue+0x30;
								TestVolt3=atof(sbuff);
								BTAB=(BTAB==2)?0:BTAB+1;
								break;
						 }
						 case 2:
						 {
						 	sprintf(sbuff,"%04.1f",StartVolt3);
								if(BTAB<2)
									sbuff[BTAB]=KeyValue+0x30;
								else
									sbuff[BTAB+1]=KeyValue+0x30;
								StartVolt3=atof(sbuff);
								BTAB=(BTAB==2)?0:BTAB+1;
								break;
						 }
					}							
...
void Manual(uint16 a)                    //手动测试
{
	...
	VTAB=0;HTAB=0x17;
	putEng('"');putEng('0');putEng('"');//0是控制正反转的
	if(STAB==0)
		verString("正转");
	else
		verString("反转");
	...
	while(1)
	{
		...
		if((KeyValue==F2)&&(!End_Test))     //F2是开始按键
		{
			...
			{					
				{	
					In_Test=0;In_pause=1;EnableF=0;
					PWM1MR0=1000;
					PWM1MR3=1000;
					PWM1LER=0x09;
				}
			...
			{	    VTAB=0;HTAB=0x15;
					putString("         ");
					Counter=0;
					RunMode=(STAB==0)?1:2;       //这里决定STAB=0是正转
					volt=StartVolt3;             //启动电压
					if(StartMotor(StartTime,1)==0)
					{	
						VTAB=0;HTAB=0x17;
	                    putEng('"');putEng('0');putEng('"');//0是控制正反转的
	                    if(STAB==0)
		                 ...
		                 verString("反转");
					    In_pause=0;EnableF=1;     //启动完成标志
						volt=TestVolt3;         //试验电压
						RunMotor(1, RunMode);
						VTAB=0x70;
		                HTAB=0x00;putString( "F1");verString("(--)");
						HTAB=0x08;putString("F2");verString( "开始");
						HTAB=0x10;putString("F3");verString( "返回");
					}
					else
					{	VTAB=0x70;
					    HTAB=0x00;putString( "F1");verString("(--)");putString("  ");
						HTAB=0x08;putString("F2");verString( "继续");	
						HTAB=0x10;putString("F3");verString( "返回");
					}
			}	
			while(KeyValue!=0xff)
...
{	uint16 ii,sCycleCount;
	...
	if(In_Test)                     //第一个字节是数据类型
		UR0_T_Buff[0]=0x31;         //在测试中
	else if (In_pause)
		 UR0_T_Buff[0]=0x32;        //暂停
	else if (End_Test)
		 UR0_T_Buff[0]=0x34;        //全部实验完成
	else if(type==6)
		 UR0_T_Buff[0]=0x36;
	else UR0_T_Buff[0]=0x30;        //等待中

	sprintf(UR0_T_Buff+1,"%05.2f",sFv);	      //电压	
	sprintf(UR0_T_Buff+6,"%05.2f",sFc);       //电流	
	if(sFp<1000)
		sprintf(UR0_T_Buff+11,"%05.1f",sFp);  //功率	
	else
		sprintf(UR0_T_Buff+11,"%05.0f",sFp);	
	sprintf(UR0_T_Buff+16,"%05.3f",sFt);      //转矩
	sprintf(UR0_T_Buff+21,"%05.1f",sFs);      //转速
	sprintf(UR0_T_Buff+26,"00000");	          //直流 默认为00000
	
	UR0_T_Buff[31]=sNowState;                 //阶段
	UR0_T_Buff[32]=0x30|(uint8)Alarm;         //报警
	if(Zf)
		UR0_T_Buff[32] |= 0x40;
	...
	{	U0THR = MyID|0x07;
		...
		{	/*
			...
			{	nn=2;
				...
				memset(TRBuff,0,sizeof(TRBuff)); 
				memcpy(TRBuff,&s_buff[nn],5); nn+=5; 						
				Cons[8]=atof(TRBuff);      //试验电压
				memset(TRBuff,0,sizeof(TRBuff)); 
				memcpy(TRBuff,&s_buff[nn],5); nn+=5; 						
				Cons[9]=atof(TRBuff);      //启动电压
				//Alarm=(uint16)s_buff[28];    //恒值中没有报警
				if(In_Test==0)
				{
					V32.f32=Cons[0];
					ISendStr(0xa0,0x80,V32.b8,4);Delay(2);
			...
			{	
				...
				memset(TRBuff,0,sizeof(TRBuff)); 
				memcpy(TRBuff,&s_buff[nn],5); nn+=5; 						
				TestVolt3=atof(TRBuff);      //试验电压
				memset(TRBuff,0,sizeof(TRBuff)); 
				memcpy(TRBuff,&s_buff[nn],5); nn+=5; 						
				StartVolt3=atof(TRBuff);      //启动电压
				return mode;								
			}
			default:
				return 0xff;
...
{	
	...
	{	
	    ...
	    showPerfomance2();			 
		DispPG11(3);
		PerforStep(0);        //RunMode会被赋值
		volt=StartVolt2/10;
		if(StartMotor(StartTime,3))  //低压启动
		{
			nowClock=0;STAB=0;In_pause=0;
			if(num==0)             //a的引入是 ConstantTest2(uint16 a)的a
				showRun0();
			else
			{   showPG00();
				DispPG00();
			...
			{	case 1:                        //由上位机操作0x91命令
				{	U0THR = MyID|0x01;
					...
					{	Bell();
						...
						HTAB=0x08;
						putString("       ");    /////////////////////////
						//RunMotor(1,RunMode);     //这里应该也不需要重启动
						//Delay(200);
						VTAB=0x70;HTAB=0x13;								
						Second1=2;
						Alarm=0;
...
{	
    float result;
	VTAB=0x36;HTAB=0x1c;
	sprintf(sbuff,"%d",(Cycle));               //显示阶段
	putBuff(sbuff);	

	Counter=0;
	sprintf(sbuff,"%5.1f",(float)Counter/10.0);//显示定时
	...
	if(a)
	{
        if(Step2[Cycle].LoadMode==0)          //0代表定值加载
        {
        	LoadMode2=0;                      //标志
			pid0.SetPoint= Step2[Cycle].LoadValue;
			if(pid0.integral<0) pid0.integral=0;	
		...
		{	LoadMode2=1;                        //标志有效
			if(pid0.runtype==0)                 //转矩
			{	SetS= Step2[Cycle].LoadValue;   //设定的加载值
				dS=(SetS-pFt)/((float)Finish);  //等分
				SetS=pFt+dS;
				pid0.SetPoint=SetS;
				pid0.SetPoint2=0;
			}
			else
			{	SetS= Step2[Cycle].LoadValue*10;
				dS=(SetS-LastLoad2)/((float)Finish);
				SetS=LastLoad2+dS;
				result=SetS;
				result=(result > 1000.0) ? 1000.0 : (result < 0.0 ? 0.0 : result);
				LastLoad2=result;
...
{		uint16 Start=0;
        float temptest,tempstart;
			VTAB=0x70;
			HTAB=0x00;putString("起动中...      ");
			if(a==3){
				temptest=TestVolt2/10;
	            tempstart=StartVolt2/10;
				}

			RunMotor(1,RunMode);
			StartCount=(uint16)sTime*10;          //实际：把S转换成Ms,然后除100
	                                              //(uint16)sTime*10个100Ms
			while(Start==0)
			{	Read_Key();
				if(KeyValue==F3)
				{	Bell();
				...
				{	FlagMs100=0;	
					...
					if(--Second1==0)
			        {	Second1=10;	              //1S
			           if(a==0)volt=volt+(Cons[8]-Cons[9])/Cons[7];
					   else if(a==1)volt=volt+(TestVolt3-StartVolt3)/StartTime;
					   else if(a==2)volt=volt+(TestVolt1-StartVolt1)/StartTime;
			           else if(a==3)volt=volt+(temptest-tempstart)/StartTime;
					   RunMotor(1, RunMode);
						}
					VTAB=0x00;HTAB=0x19;
					sprintf(sbuff,"%5.1f",(float)Counter/10);
