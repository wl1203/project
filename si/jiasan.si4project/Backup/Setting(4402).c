...
{	uint16 nn,ii;
  ...
  if(a==0)                    //出厂工艺
	...
	for(ii=0;ii<10;ii++)            //10个阶段
	{
		...
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],5);nn+=5;   
		TestVolt1 = atof(TRBuff)*100;
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],5);nn+=5;   
		StartVolt1 = atof(TRBuff)*100;
		memset(TRBuff,0,sizeof(TRBuff)); 
	...
	{	
	    // Step[5].endvalue=Step[5].endvalue<10?10:Step[5].endvalue;     //不足10s强行设定10s
	      if(Step[0].endvalue)
			Step[4].endvalue=0.5;    //插入了第5个阶段的时间	
	}
	else
	...
	{	nn=0;
		...
		LoadType2=(s_buff[nn]==0x35)?5          //加载方式   
			:(s_buff[nn]==0x37)?7:0;
		nn=nn+1;
		...
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],3);nn+=3;   
		TestVolt2 = atoi(TRBuff)*10;
		memset(TRBuff,0,sizeof(TRBuff)); 
		memcpy(TRBuff,&s_buff[nn],3);nn+=3;   
		StartVolt2 = atoi(TRBuff)*10;
		memset(TRBuff,0,sizeof(TRBuff)); 
