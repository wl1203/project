...
U32 		Wbuf5460V,Rbuf5460V,Wbuf5460M,Rbuf5460M,b;
...
STEP 	Step[16];
STEP2  Step2[22];
uint16 LoadType2=0,LastStep2=0,voltdir=0;
float LoadVolt2,volt;
IN_ROT 	in_rot;
...
volatile uint16 FlagMs2=0,FlagMs100=0,mS100=0,mS005=0,FlagS1=0;
...
{	
	...
	volt = volt*100;
	sprintf(b,"%04d",(uint32)volt); //定义的U32联合体 类型的b         uint32 无符号32位
	U3THR=0xEB;
	U3THR=0x90;
	U3THR=Motor_SW;
	U3THR=(uint8)(b[0]&0x0f);
	U3THR=(uint8)(b[1]&0x0f);
	U3THR=(uint8)(b[2]&0x0f);
	U3THR=(uint8)(b[3]&0x0f);
...
/*
	...
	*/
	...
	{
		Read_Key();	
		if(FlagBT)               //FlagBT   Button 的改变都是在读键函数中 
		{	FlagBT=0;
		...
		{                        
			Bell();
			if(In_Test!=0) //要求暂停      End_Test=0说明还没有结束测试
			{	In_pause=0;
				End_Test=1;
				In_Test=0;
				PWM1MR0=1000;
...
{		float result;		
		...
		{	case 0:                              //第一阶段
			{	pid0.SetPoint= Step[Cycle].loadvalue;
				RunMotor(0,1);RunMode = 1;	     //RunMode = 1  正转
				StartCount=10;                   //延迟10s
				break;			}
