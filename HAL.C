...
extern volatile uint32	PosiCount,PosiIn,LastPosiIn,Position;
...
{	PCONP = PCGPIO_Enable;	
	...
	FIO2DIR = 0x00000578;					 /* KEYIN3/KEYIN2/KEYIN1/VOLT_RST/TORQ_CS*/
	FIO2SET = 0xffffffff;
...
void Read_Posi(void)
{
	...
	FIO2CLR |= KEY_IN4;
	FIO2CLR |= KEY_IN4;
	KtempB1= (~FIO1PIN )& DATABUSp;
	FIO2SET |= KEY_IN4;
	FIO2SET |= KEY_IN4;

	PosiIn=0;
	if(KtempB1&0x0002)	PosiIn	|= 0x01;		//d K
	if(KtempB1&0x0100)	PosiIn 	|= 0x02;
	//else if(KtempB1&0x0400)	PosiIn |= 0x04;
	//else if(KtempB1&0x8000)	PosiIn |= 0x08;	
	//else PosiIn=0;
	
	if (PosiIn==LastPosiIn)
	{
		Position=LastPosiIn;
	}
	else
